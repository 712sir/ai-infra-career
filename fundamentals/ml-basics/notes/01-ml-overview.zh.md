# 第01讲：面向基础设施工程师的机器学习概述

## 目录
1. [Introduction](#introduction)2. [What is Machine Learning?](#what-is-machine-learning)3. [The ML Workflow](#the-ml-workflow)4. [Types of Machine Learning](#types-of-machine-learning)5. [Common ML Tasks](#common-ml-tasks)6. [ML System Architecture](#ml-system-architecture)7. [Infrastructure Implications](#infrastructure-implications)8. [Summary](#summary)
---
...
构建 ML 基础设施不必是机器学习专家，但需要理解那些"黑盒子"里发生了什么。
打个比方：
...
本讲提供 AI 基础设施工程师所需的 ML 知识——不多不少，刚好够用。
### 学习目标

学完本讲后，你将能够：- Understand what machine learning actually does (demystifying the hype)
...
**Problems with this approach:**- Rules are brittle and easily circumvented
...
**Advantages:**- Learns complex patterns humans can't easily articulate
...
**Machine learning:**```
样本（数据）→ 学习函数 → 应用到新输入 → 输出
```

**Concrete example - Image classification:**
```
传统方式：如果像素匹配模式 X 则判定为猫
ML 方式：给你 10000 张猫的图片和 10000 张狗的图片
...
✅ **Pattern recognition** - Identifying cats in images, detecting fraud✅ **Prediction** - Stock prices, equipment failure, customer churn✅ **Classification** - Spam detection, sentiment analysis, disease diagnosis✅ **Generation** - Text generation (GPT), image generation (DALL-E)✅ **Complex decision-making** - Game playing (AlphaGo), robotics
### What ML Is NOT Good For

❌ **Exact calculations** - Use traditional programming (2 + 2 = 4)❌ **Guaranteed correctness** - ML is probabilistic, not deterministic❌ **Explaining decisions** - Many models are "black boxes"❌ **Tasks without data** - ML needs examples to learn from❌ **Simple rule-based logic** - Don't use ML when `if/else` suffices
---

## The ML Workflow

### 概述: Data to Deployed Model
...
**What happens:**- Gather training examples
...
**Infrastructure role:**- Provide storage for datasets
- Build data pipelines
- Ensure data access for training

### Phase 2: Data Preparation

**What happens:**- Clean data (remove duplicates, handle missing values)
...
**Infrastructure role:**- Provide compute for data processing
- Store processed datasets
- Version control datasets

### Phase 3: Training (The "Learning" Part)

**What happens:**- Model processes training data
...
**Infrastructure role (CRITICAL):**- Provide GPU/TPU compute for training
...
**What happens:**- Test model on unseen data (test set)
...
**Infrastructure role:**- Provide compute for evaluation
- Store evaluation results
- Track model versions and metrics

### Phase 5: Deployment

**What happens:**- Package trained model
- Deploy to serving infrastructure
- Expose API for predictions
- Implement monitoring and logging

**Infrastructure role (YOUR PRIMARY JOB):**- Build model serving infrastructure
...
**What happens:**- Receive new input
...
**Infrastructure role:**- Serve predictions with low latency
...
**What happens:**- Monitor model performance in production
- Detect data drift (input distribution changes)
- Detect concept drift (relationships change)
- Trigger retraining when needed

**Example drift detection:**```
第 1 周：平均预测置信度 0.92 ✓ 正常 ✓
第 2 周：平均预测置信度 0.90 ✓ 正常 ✓
第 3 周：平均预测置信度 0.85 ⚠️ 下降，需要调查 ⚠️
Week 4: Average prediction confidence: 0.75 ❌ (Retrain!)
```

**Infrastructure role:**- Implement monitoring dashboards
...
**Common tasks:**- Classification (output is a category)
- Regression (output is a number)

**Infrastructure considerations:**- Requires labeled training data (can be expensive to create)
...
**Common tasks:**- Clustering (group similar examples)
- Dimensionality reduction (compress data)
- Anomaly detection (find outliers)

**Infrastructure considerations:**- No labels needed (cheaper data)
...
**Common tasks:**- Game playing (Chess, Go, video games)
- Robotics (learn to walk, grasp objects)
- Resource optimization (data center cooling)

**Infrastructure considerations:**- Requires simulation environment (expensive)
...
**Infrastructure considerations:**- Reduces labeling cost
...
**Examples:**- Email: spam or not spam
- Image: cat, dog, bird, etc.
- Medical: disease diagnosis
- Sentiment: positive, negative, neutral

**Output:** Class label (category)
**Infrastructure needs:**- **Training:** Moderate to high compute (GPUs for complex tasks)
...
**Infrastructure needs:**- **Training:** Moderate compute
...
**Infrastructure needs:**- **Training:** High compute (GPUs required)
...
**Infrastructure needs:**- **Training:** Very high compute (large models need multiple GPUs)
...
**Examples:**- Netflix movie recommendations
- Amazon product suggestions
- Spotify music playlists
- YouTube video recommendations

**Infrastructure needs:**- **Training:** High compute and memory (large datasets)
...
**Training (happens occasionally):**- Processes entire dataset (millions of examples)
...
**Inference (happens constantly):**- Processes single inputs (or small batches)
...
**Training costs:**- GPU hours: $1-$30+ per hour depending on GPU type
- Storage: $0.02-$0.10 per GB/month
- Data processing: CPU compute and I/O costs

**Inference costs:**- Per-request: $0.0001 - $0.01+ per prediction
...
1. **ML learns patterns from data** rather than explicit programming2. **Training and inference are fundamentally different** - understand this distinction!3. **Different ML types** (supervised, unsupervised, RL) have different infrastructure needs4. **Common tasks** (classification, regression, NLP) have predictable resource requirements5. **ML systems are complex** - many components beyond just "the model"6. **Infrastructure is critical** - models are useless without good infrastructure
### Infrastructure Engineer's Mental Model

When you encounter an ML project, ask:
✅ **Is this training or inference?**✅ **What type of ML task is this?**✅ **What are the resource requirements?**✅ **What's the latency requirement?**✅ **How often does the model need to be retrained?**✅ **What's the expected query volume?**✅ **What framework is used? (PyTorch, TensorFlow, etc.)**
### What You've Learned
...
**Lecture Version**: 1.0**Last Updated**: October 2025**Word Count**: ~3,800 words**Estimated Reading Time**: 50-70 minutes
